const Header = () => {
  return (
    <header className="header">
    <nav>
        <div className="headerline1">
            <div className="mllogo">
            <svg width="134" height="34" viewBox="0 0 134 34" fill="none" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
                <rect width="134" height="34" fill="url(#pattern0)"/>
                <defs>
                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlinkHref="#image0_1_6" transform="scale(0.00746269 0.0294118)"/>
                    </pattern>
                    <image id="image0_1_6" width="134" height="34" xlinkHref="data:image/png;base64,"/>
                </defs>
            </svg>
            </div>
            <div className="input">
            <form action="">
                <label htmlFor="">
                    <input placeholder="Buscar productos, marcas y más..." name="busqueda"/>
                </label>
            </form>
        </div>
    </div>
    <div className="headerline2">
        <div>
            <span className="texto1">Ingresa tu domicilio</span>
            <span className="texto2">CATEGORÍAS</span>
            <span className="texto3">Ofertas</span>
            <span className="texto3">Historial</span>
            <span className="texto4">Supermercado</span>
            <span className="texto3">Moda</span>
            <span className="texto3">Vender</span>
            <span>Ayuda / PQR</span>
        </div>
        <div>
            <span>Crea tu cuenta  Ingresa  Mis Compras</span>
        </div>
    </div>
    </nav>
</header>
  )
}

export { Header }